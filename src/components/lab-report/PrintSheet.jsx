...
export default function PrintSheet({ currentResult, resetAnalyzer }) {
  return (
    <div className="space-y-8 fade-in">
      ...
      <div className="glass-card p-6 sm:p-10 rounded-3xl border border-white/10 shadow-2xl space-y-8 bg-[#0a192f]/45">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b-2 border-cyan-500/30 pb-6 gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-white/50">
              ISO 9001:2015 Certified Diagnostic Services
            </p>
          </div>
          <div className="text-left sm:text-right text-xs text-white/50">
            <p>
              ...
              <span className="font-mono text-cyan-300 font-bold">
                VDX-{Math.floor(100000 + Math.random() * 900000)}
              </span>
            </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 bg-white/5 border border-white/10 p-5 rounded-2xl">
          <div>
            <span className="block text-[10px] uppercase font-bold text-white/40 tracking-wider">
              Patient Name
            </span>
            <span className="text-sm font-semibold text-white">
              {currentResult.details.patientName}
            </span>
          </div>
          <div>
            <span className="block text-[10px] uppercase font-bold text-white/40 tracking-wider">
            ...
            <span className="text-sm font-semibold text-white">
              {currentResult.details.patientAge} /{" "}
              {currentResult.details.patientGender}
            </span>
          </div>
          <div className="col-span-2 sm:col-span-1">
            ...
            <span className="text-sm font-semibold text-cyan-400">
              {currentResult.details.reportCategory}
            </span>
          </div>
        </div>

        {/* Parameters Table if Logged Manually */}
        {currentResult.loggedParameters &&
          currentResult.loggedParameters.length > 0 && (
                    ...
                    {currentResult.loggedParameters.map((p, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4">
                            {isHigh ? (
                              <span className="px-2.5 py-1 rounded-md bg-red-500/10 border border-red-500/20 text-red-400 text-[10px]">
                                ▲ HIGH
                              </span>
                            ) : isLow ? (
                              <span className="px-2.5 py-1 rounded-md bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-[10px]">
                                ▼ LOW
                              </span>
                            ) : (
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-white font-outfit border-b border-white/5 pb-2">
            AI Diagnostic Analysis & Suggestions
          </h3>
      ...
      <div className="flex justify-center gap-4 no-print">
        <button
          ...
          className="btn-primary py-3.5 px-8 text-base shadow-lg shadow-cyan-500/20"
        >
